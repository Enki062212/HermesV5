...
function LandingPage() {
  ...
  const USE_CASES=[
    {label:"E-Commerce",icon:"🛍️",desc:"Streamline management, automation & revenue growth"},
            ...
            {activePreview==="AI Chatbot" && (
              <div>
                ...
                <div style={{marginTop:16,display:"flex",gap:10,flexWrap:"wrap"}}>
                  {[["0.8s","Avg Response"],["99.98%","Uptime"],["7","Platforms"],["24/7","Always On"],["Taglish","Language"],["∞","Conversations"]].map(([v,l])=>(
...
function HermesChatbot() {
  ...
  - ✉️ Unified inbox for Facebook, Instagram, TikTok, Shopee, Lazada, Viber
  ...
  - 🏛️ Enterprise: ₱25,000/mo (monthly) or save 50% annually — unlimited everything
  ...
  const send=async(text)=>{
    ...
    }catch{ setMsgs(prev=>[...prev,{role:"assistant",content:"⚠️ Connection error. Please try again or email admin@exponify.ph."}]); }
    setLoading(false);
  };
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 16 }}>
        ...
        <GoldKPI label="Critical Stock" value={products.filter(p => p.status === "critical").length} change="Reorder now" up={false} icon="⚠️" color={C.red} />
      </div>
  ...
  return (
    <div style={{flex:1,overflowY:"auto",padding:"20px 24px"}} className="demographics-module">
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}} className="kpi-grid">
        ...
        <GoldKPI label="Female Audience" value="61.4%" change="+2.1% MoM" up icon="♀️" color={C.rose}/>
        <GoldKPI label="Top Region" value="Metro MNL" change="38.4% share" up icon="🗺️" color={C.cyan}/>
      </div>
      ...
      <div style={{background:C.bg3,border:`1px solid ${C.border}`,borderRadius:14,padding:"20px 22px",marginBottom:14}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}} className="archetypes-grid">
          {[
            ...
            {name:"Value Hunters",pct:28,icon:"🛍️",color:C.amber,traits:["Ages 25–44","Mixed gender","Price-sensitive","Shopee-native"],ltv:"₱1.4M"},
          ...
          <>
            <div style={{padding:"16px 22px",borderBottom:`1px solid ${C.border}`,background:C.bg2}}>
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
                ...
                {sel.urgent&&!sel.replied&&<GoldTag color={C.red} style={{marginLeft:"auto"}}>⚠️ Urgent</GoldTag>}
              </div>
              <div style={{
              ...
              <div style={{ background: C.bg4, border: `1px dashed ${C.border2}`, borderRadius: 10, padding: "14px", marginTop: 10 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                  <div>
                    ...
                    <input value={newBtnTitle} onChange={e => setNewBtnTitle(e.target.value)} maxLength={20} placeholder="e.g. 🛍️ Our Products" style={inp} />
                  </div>
                  <div>
                ...
                <div style={{ marginBottom: 8 }}>
                  ...
                  <textarea value={newBtnReply} onChange={e => setNewBtnReply(e.target.value)} rows={2} placeholder="e.g. Here are our products! 🛍️ We have cakes starting at ₱350..." style={{ ...inp, resize: "none" }} />
                </div>
                <div style={{ marginBottom: 10 }}>
...
function EmailMarketingModule({ activeBrandId, activeBrandName, notify, session: _session }) {
  ...
  const runAutomation=async()=>{
    ...
    log("✍️ Generating personalized email content...");if(!subject)setSubject(`${activeBrandName||"Exponify"} — Exclusive Offer for ${GOALS.find(g=>g.id===campaignGoal)?.label||"Your Brand"}`);await new Promise(r=>setTimeout(r,400));await aiDraftEmail();await new Promise(r=>setTimeout(r,600));log("✅ Email content generated with AI","success");
          ...
          <div style={{width:280,flexShrink:0}}>
            <div style={{background:C.bg3,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px 18px",position:"sticky",top:0}}>
              ...
              <div style={{background:sel.roas>=4?`${C.green}12`:`${C.amber}12`,border:`1px solid ${sel.roas>=4?C.green:C.amber}33`,borderRadius:10,padding:"12px 14px",textAlign:"center"}}>
                ...
                <div style={{marginTop:4,fontSize:10,fontWeight:700,color:sel.roas>=4?C.green:sel.roas>=2.5?C.amber:C.red}}>{sel.roas>=4?"🔥 Excellent ROAS":sel.roas>=2.5?"✅ Good ROAS":"⚠️ Needs Optimization"}</div>
              </div>
            </div>
          </div>
        )}
